# بررسی معماری و تسک‌های کامل پروژه

---

## بخش اول: بررسی معماری پیشنهادی

معماری پیشنهادی شما کلاً خوب و اصولی است اما چند نکته برای بهبود دارم:

### نقاط قوت
- جداسازی درست `src`، `api`، `scripts`، `tests`
- وجود `configs` برای مدیریت تنظیمات
- وجود `ui` جدا از `api`

### مشکلات و پیشنهاد اصلاح

**مشکل ۱:** پوشه `cache_store` در ریشه پروژه نامناسب است. داده‌های runtime نباید کنار کدها باشند.

**مشکل ۲:** در `scripts/model_selection` فقط یک فایل است. این می‌تواند داخل `evaluation` باشد یا به شکل مستقل‌تری ساختار بگیرد.

**مشکل ۳:** پوشه `resources` مبهم است. بهتر است به `assets` تغییر نام پیدا کند یا محتوای آن وارد `data` شود.

**مشکل ۴:** `scripts/training` و `scripts/quantization` بهتر است زیرپوشه مستقیم `scripts` بمانند اما فایل `prepare_dataset.py` منطقاً به مرحله اول تعلق دارد و باید از `training` جدا شود.

**مشکل ۵:** در `src/arabic_summarizer/inference` نیاز به `llama_cpp_engine.py` هم داریم چون پروپوزال هر دو موتور `llama.cpp` و `ONNX` را مشخص کرده.

**مشکل ۶:** پوشه‌ای برای `evaluation` داخل `src` وجود ندارد در حالی که کد محاسبه ROUGE و BERTScore باید به عنوان ماژول قابل استفاده مجدد وجود داشته باشد.

### معماری اصلاح‌شده نهایی
...
**دلایل انتخاب:**
- `ara`: مخفف Arabic، کوتاه و بین‌المللی
- `sum`: مخفف Summarization
- `offline`: ویژگی اصلی و متمایزکننده محصول
- کوتاه، قابل تلفظ، قابل جستجو و حرفه‌ای

**گزینه‌های جایگزین:**
```
arabic-offline-summarizer      # توصیفی‌تر
arabart-summarizer-offline     # نام مدل محور
bitforge-arasum               # نام تیم + محصول
```
...
**MIT License** توصیه می‌شود.

**دلایل:**
- پروپوزال مشخص کرده که «تیم مجری مالکیت سورس کد را دارا می‌باشد» پس می‌توان آزادانه لایسنس انتخاب کرد
- MIT ساده‌ترین و شناخته‌شده‌ترین لایسنس open source است
- کارفرما (سازمان فرهنگ و ارتباطات) می‌تواند کد را بدون محدودیت استفاده کند
- در آینده اگر پروژه را توسعه دادید و فروختید مشکلی ایجاد نمی‌کند
- اگر بخواهید مدل را روی HuggingFace منتشر کنید با MIT مشکلی نخواهید داشت

**اگر می‌خواهید مصرف تجاری توسط دیگران را محدود کنید** از **Apache 2.0** استفاده کنید که patent protection هم دارد.